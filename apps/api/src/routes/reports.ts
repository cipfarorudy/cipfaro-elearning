...
router.get(
  ...
  async (req, res) => {
    ...
    const html = await ejs.renderFile(
      __dirname + "/../templates/attendance.ejs",
      {
        session,
        rows,
        hash,
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="attendance_${sessionId}.pdf"`
    );
    res.setHeader("X-Data-Hash", hash);
    res.send(pdf);
  }
...
router.get(
  ...
  async (req, res) => {
    ...
    const completed = cmi.filter((x) =>
      (x.lessonStatus || "").match(/completed|passed/i)
    ).length;
    ...
    const html = await ejs.renderFile(
      __dirname + "/../templates/attestation.ejs",
      {
        user: enr.user,
        session: enr.session,
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="attestation_${enrollmentId}.pdf"`
    );
    res.setHeader("X-Data-Hash", hash);
    res.send(pdf);
  }
...
router.get(
  ...
  async (req, res) => {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="attestations_${sessionId}.zip"`
    );
    archive.pipe(res);

    for (const enr of session.enrollments) {
      const cmi = await prisma.scormCmi.findMany({
      ...
      const pdf = await htmlToPdfBuffer(html);
      archive.append(pdf, {
        name: `attestation_${enr.user.lastName || ""}_${
          enr.user.firstName || ""
        }_${enr.id}.pdf`.replace(/\s+/g, "_"),
      });
    }
